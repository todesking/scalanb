...
object Cacheable extends CacheableLowPriority {
  ...
  def apply[A](ser: (FileSystem, String, A) => Unit)(de: (FileSystem, String) => A): Cacheable[A] = new Cacheable[A] {
    override def save(fs: FileSystem, name: String)(value: A) = ser(fs, name, value)
    override def load(fs: FileSystem, name: String) = de(fs, name)
  }

  def byJavaSerialization[A](doPut: (A, ObjectOutputStream) => Unit)(doGet: ObjectInputStream => A): Cacheable[A] =
    Cacheable[A] { (fs, name, value) =>
